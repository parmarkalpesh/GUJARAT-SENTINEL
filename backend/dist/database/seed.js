...
const cameraTypes = ['IP', 'PTZ', 'DOME', 'BULLET', 'IP'];
const protocols = ['RTSP', 'ONVIF', 'RTSP'];
// ── Sample vehicles ──
...
async function seed() {
  ...
  `);
    ...
    const operatorHash = bcryptjs_1.default.hashSync('operator123', 10);
    const users = [
        { id: (0, uuid_1.v4)(), username: 'admin', email: 'admin@sentinel.guj.gov.in', password_hash: passwordHash, full_name: 'System Administrator', role: 'SUPER_ADMIN', department: 'Home Department' },
        { id: (0, uuid_1.v4)(), username: 'sp_ahmedabad', email: 'sp.ahmedabad@sentinel.guj.gov.in', password_hash: passwordHash, full_name: 'SP Ahmedabad', role: 'POLICE_ADMIN', department: 'Gujarat Police' },
        { id: (0, uuid_1.v4)(), username: 'operator1', email: 'operator1@sentinel.guj.gov.in', password_hash: operatorHash, full_name: 'Control Room Operator 1', role: 'CONTROL_ROOM_OPERATOR', department: 'Home Department' },
        { id: (0, uuid_1.v4)(), username: 'investigator1', email: 'inv1@sentinel.guj.gov.in', password_hash: operatorHash, full_name: 'Inspector R. Patel', role: 'INVESTIGATOR', department: 'Gujarat Police' },
        { id: (0, uuid_1.v4)(), username: 'rto_user', email: 'rto@sentinel.guj.gov.in', password_hash: operatorHash, full_name: 'RTO Officer', role: 'DEPARTMENT_USER', department: 'RTO' },
        { id: (0, uuid_1.v4)(), username: 'viewer', email: 'viewer@sentinel.guj.gov.in', password_hash: operatorHash, full_name: 'View Only User', role: 'VIEW_ONLY', department: 'General' },
    ];
    ...
    for (const d of departments) {
        insertDept.run((0, uuid_1.v4)(), d.name, d.code, `${d.name} CCTV Operations`);
    }
    // ── Seed 50 Cameras ──
  ...
  `);
    ...
    for (let i = 0; i < 50; i++) {
        ...
        insertCamera.run((0, uuid_1.v4)(), cameraId, `${loc.name} Camera`, dept.name, loc.name, loc.district, loc.lat, loc.lng, cameraType, vendor, `${vendor}-${cameraType}-${camNum}`, `192.168.${Math.floor(i / 255)}.${(i % 255) + 1}`, protocol, vms, `NVR-${loc.district.substring(0, 3).toUpperCase()}-${Math.ceil((i + 1) / 5)}`, i % 3 === 0 ? '4K' : i % 3 === 1 ? '1080p' : '720p', i % 2 === 0 ? 30 : 25, i % 4 === 0 ? 'CLOUD' : i % 4 === 1 ? 'NVR' : i % 4 === 2 ? 'LOCAL' : 'HYBRID', i % 3 === 0 ? 30 : i % 3 === 1 ? 15 : 7, `2024-${String((i % 12) + 1).padStart(2, '0')}-15`, i % 5 === 0 ? 'EXPIRED' : 'ACTIVE', status, health, 'ANPR,VEHICLE_DETECTION,OBJECT_DETECTION', isOffline ? null : new Date().toISOString(), `rtsp://192.168.${Math.floor(i / 255)}.${(i % 255) + 1}:554/stream1`);
    }
    // ── Seed Watchlist ──
  ...
  `);
    ...
    for (const w of watchlistData) {
        insertWatchlist.run((0, uuid_1.v4)(), w.wid, w.category, 'VEHICLE', w.vehicle, w.type, w.color, w.case, 'ACTIVE', w.priority, 'Gujarat Police', w.notes);
    }
  ...
  `);
    ...
    for (const det of primaryJourney) {
        ...
        if (cam) {
            insertDetection.run((0, uuid_1.v4)(), `DET-${(0, uuid_1.v4)().substring(0, 8).toUpperCase()}`, det.cam, 'GJ01AB1234', 'GJ01AB1234', det.time, cam.lat, cam.lng, cam.location, cam.district, 'Sedan', 'White', det.conf, det.dir, 1, 'WL-2026-001');
        }
    }
    ...
    for (const vehicle of sampleVehicles) {
        ...
        for (let d = 0; d < numDetections; d++) {
            ...
            insertDetection.run((0, uuid_1.v4)(), `DET-${(0, uuid_1.v4)().substring(0, 8).toUpperCase()}`, cam.cameraId, vehicle.plate, normalizePlate(vehicle.plate), timestamp, cam.lat, cam.lng, cam.location, cam.district, vehicle.type, vehicle.color, 85 + Math.random() * 14, ['NB', 'SB', 'EB', 'WB'][Math.floor(Math.random() * 4)], isWatchlisted ? 1 : 0, watchId);
            detectionCount++;
        }
    }
  ...
  `);
    ...
    for (const a of alertsData) {
        ...
        if (cam) {
            insertAlert.run((0, uuid_1.v4)(), a.aid, a.type, a.priority, a.cam, `${cam.location} Camera`, cam.location, cam.district, cam.lat, cam.lng, a.time, a.entity, 'VEHICLE', a.conf, a.wid, a.wcat, a.status);
        }
    }
    // ── Seed Incidents ──
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    insertIncident.run((0, uuid_1.v4)(), 'INC-2026-0001', 'ALT-2026-0192', 'Stolen Vehicle Detected — GJ01AB1234', 'CRITICAL', 'SG Highway Junction', 'Ahmedabad', 'Inspector R. Patel', 'INVESTIGATING', 'Stolen white Honda City (GJ01AB1234) detected on SG Highway Junction camera at 10:05:22. Vehicle has been tracked moving northward through Gandhinagar and Mehsana. Currently last seen near Patan. All units alerted.', 'GJ01AB1234', JSON.stringify([
  ...
  `);
    ...
    for (const e of eventsData) {
        insertEvent.run((0, uuid_1.v4)(), `EVT-${(0, uuid_1.v4)().substring(0, 8).toUpperCase()}`, e.type, e.source, e.sid, e.desc, null, e.time);
    }
    // ── Seed Integrations ──
  ...
  `);
    ...
    for (const i of integrationsData) {
        insertIntegration.run((0, uuid_1.v4)(), i.name, i.type, i.status, i.sync, i.records, i.endpoint, i.desc);
    }
    // ── Seed System Metrics ──
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    insertMetrics.run((0, uuid_1.v4)(), 41, 62, 74, 58, 1250, 890, 47, 31, 12, 180, new Date().toISOString());
    // ── Summary ──
    const counts = {
        users: db.prepare('SELECT COUNT(*) as c FROM users').get().c,
        cameras: db.prepare('SELECT COUNT(*) as c FROM cameras').get().c,
        detections: db.prepare('SELECT COUNT(*) as c FROM vehicle_detections').get().c,
        watchlist: db.prepare('SELECT COUNT(*) as c FROM watchlist_entries').get().c,
        ...
        integrations: db.prepare('SELECT COUNT(*) as c FROM integrations').get().c,
    };
    console.log('\n✅ Seed complete!');
    console.log('─────────────────────────────────');
